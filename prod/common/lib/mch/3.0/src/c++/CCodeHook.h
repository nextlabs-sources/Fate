...
#ifdef _WIN64
  #define CTrampSize 54
#else
  #define CTrampSize 30
#endif
...
class SYSTEMS_API CCodeHook
{
  ...
  protected:
    ...
    void Initialize(HMODULE hModule, LPCSTR apiName, LPVOID hookFunction,
                    LPVOID callbackFunction, LPVOID *nextHook, DWORD flags);
    BOOL DoNewHookPrep(DWORD flags, LPVOID *pa, DWORD *error, bool *keepMapFile, HANDLE *hMap);
    void InitializeQueue(HANDLE *phMap, LPCSTR mapName, LPVOID p, bool keepMapFile);
