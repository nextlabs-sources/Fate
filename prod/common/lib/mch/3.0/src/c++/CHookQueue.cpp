  ...
  if (mHMap!= NULL)
  {
    ...
    if (mpMapBuffer != NULL)
    {
      ...
      mpHookQueueHeader->NewCode = CAbsoluteJump;
        ...
        ULONG_PTR newTarget = (ULONG_PTR) tramp + CTrampSize - ((ULONG_PTR) mpHookQueueHeader->pPatchAddress + 6);
        mpHookQueueHeader->NewCode.Target = (DWORD) newTarget;
        ASSERT(mpHookQueueHeader->NewCode.Target == newTarget);
      #else
        mpHookQueueHeader->NewCode.Target = (DWORD) ((ULONG_PTR) tramp + CTrampSize);
      #endif
      *((LPVOID *) ((ULONG_PTR) tramp + CTrampSize)) = tramp;

      mpHookQueueHeader->OldCode = *mpHookQueueHeader->pPatchAddress;

      // Add initial entries
      AddEntry(NULL, (LPVOID *) ((ULONG_PTR) tramp + CTrampSize));
      AddEntry(tramp, NULL);
    }
    else
...
void CHookQueue::Initialize(HANDLE hMap, LPVOID *pTramp)
{
...
bool CHookQueue::Allocate(void)
{
  ...
  *mpMapBuffer = VirtualAlloc2(headerSize + arraySize);
  ...
  if (mpBuffer != NULL)
  {
    MoveMemory(*mpMapBuffer, mpBuffer, headerSize + mpHookQueueHeader->Capacity * sizeof(HOOK_QUEUE_HEADER));
...
// Called by Enqueue(mpCallbackFunction, (LPVOID *) mpHookStub->Target);
//   mpHookStub is an INDIRECT_ABSOLUTE_JUMP
