...
#define CELOG_CUR_FILE CELOG_FILEPATH_PROD_PEP_ENDPOINT_ADOBEPEP_PLUGIN_SAMPLESADOBEPEP_SRC_OVERLAY_CPP
...
{ 
	if(hwnd != NULL)
	{
		...
		if(boost::algorithm::iends_with(strWinCaption,ADOBE_READER_SUFFIX) || boost::algorithm::icontains(strWinCaption,ADOBE_BAT_SUFFIX) || 0 == _wcsicmp(strWinCls,L"IEFrame"))
		{
...
HWND COverLay::GetPdfViewHandle(const wstring & strFilePath)
{
	...
	{
		_snwprintf_s(theAdobeInfo.strFilePath,len, _TRUNCATE, L"%s",strPath.c_str());
...
void COverLay::DoViewOL(nextlabs::Obligations& obs,const wstring &strFilePath)
{
	...
	if (hwnd != NULL&&bRightWnd)
	{
		...
		if (pa_labeling_mod == NULL)
		{
			...
			if( pa_labeling_mod == NULL )	return ;
		...
		if(0 != lRt)
		{
			CELOG_LOGA(CELOG_DEBUG, "DoVisualLabeling fail!\n");
			return;
		}
	...
	{
		CELOG_LOGA(CELOG_DEBUG, "Get PDF VIEW is NULL\n");
	}
}
...
FuncPrintVL COverLay::GetPrintVLFunAddr()
{
	...
	{
		CELOG_LOGA(CELOG_DEBUG, "load overlay dll failed");
	}
	...
	for( it = ob_list.begin() ; it != ob_list.end() ; ++it )
	{
		if( boost::algorithm::iequals(it->name,OBLIGATION_PRINT_OVERLAY) )
		{
			...
			if (_wcsicmp(m_OverLayInfo.strTextValue,L"")!=0 && _wcsicmp(m_OverLayInfo.strFilePath,L"")!=0)
			{
				...
				COverlayTool::ConvertTxT(m_OverLayInfo.strTextValue,m_OverLayInfo.strFilePath,m_OverLayInfo.strPolicyName,m_OverLayInfo.strDateFormat,m_OverLayInfo.strTimeFormat,tempString);
