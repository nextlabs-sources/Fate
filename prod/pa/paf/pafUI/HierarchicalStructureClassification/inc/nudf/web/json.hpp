...
#define JSON_DEPRECATED(x) __declspec(deprecated(x))
...
public:
    ...
    static value __cdecl number(double value);
    ...
    /// <param name="value">The C++ value to create a JSON value from</param>
    /// <returns>A JSON number value</returns>
    static value __cdecl number(int32_t value);
    static value __cdecl number(int64_t value);
    static value __cdecl number(uint32_t value);
    static value __cdecl number(uint64_t value);
    ...
    static value __cdecl boolean(bool value);
    ...
    /// specified string has characters that should be properly escaped in JSON.
    /// <remarks>
    static value __cdecl string(std::wstring value);
    ...
    /// This overload has O(1) performance.
    /// </remarks>
    static value __cdecl string(std::wstring value, bool has_escape_chars);

private:
    // Only used internally by JSON parser.
    static value __cdecl string(const std::string &value);
...
public:
    ...
    static json::value __cdecl object(bool keep_order = false);
    ...
    static json::value __cdecl object(std::vector<std::pair<std::wstring, value>> fields, bool keep_order = false);

    /// <summary>
    /// Creates an empty JSON array
    /// </summary>
    /// <returns>An empty JSON array value</returns>
    static json::value __cdecl array();
    ...
    static json::value __cdecl array(size_t size);
    ...
    /// <returns>A JSON array value</returns>
    static json::value __cdecl array(std::vector<value> elements);

    /// <summary>
    /// Accesses the type of JSON value the current value instance is
    /// </summary>
    /// <returns>The value's type</returns>
    json::value::value_type __cdecl type() const;
    ...
    /// <returns>The parsed object. Returns json::value::null if failed</returns>
    static value parse(const std::wstring&, std::error_code&);
    ...
    /// </summary>
    /// <returns>A string representation of the value</returns>
    JSON_DEPRECATED("This API is deprecated and has been renamed to avoid confusion with as_string(), use ::json::value::serialize() instead.")
    ...
    static value parse(std::wistream &input);
    ...
    /// <param name="errorCode">If parsing fails, the error code is greater than 0</param>
    /// <returns>The parsed object. Returns json::value::null if failed</returns>
    static value parse(std::wistream &input, std::error_code& error);
    ...
    void serialize(std::wostream &stream) const;
    ...
    /// <param name="errorCode">If parsing fails, the error code is greater than 0</param>
    /// <returns>The parsed object. Returns json::value::null if failed</returns>
    static value parse(std::istream& stream, std::error_code& error);
    ...
    JSON_DEPRECATED("This API is deprecated and will be removed in a future release, use json::value::at() instead.")
        value get(const std::wstring &key) const;
    ...
    json::value& at(const std::wstring& key);
    ...
    /// <param name="key">The key of an element in the JSON object.</param>
    /// <returns>If the key exists, a reference to the value.</returns>
    const json::value& at(const std::wstring& key) const;
...
private:
    ...
    value & operator [] (const std::string &key)
    {
        ...
        return operator[](NX::utility::conversions::to_string(key));
    }
...
public:
    ...
    private:
        friend class json::details::_Object;
        friend class json::details::_Array;
        ...
        void format(std::basic_string<wchar_t> &string) const;
        /// <summary>
...
public:
    json_exception() {}
    json_exception(const wchar_t * const &message) : _message(utility::conversions::to_utf8string(message)) { }
...
public:
    ...
    json::value& at(size_type index)
    {
        if (index >= m_elements.size())
            throw json_exception(L"index out of bounds");
...
class object
{
    typedef std::vector<std::pair<std::wstring, json::value>> storage_type;
...
public:
    ...
    json::value& at(const std::wstring& key)
    {
        ...
        if (iter == m_elements.end() || key != (iter->first))
            throw json::json_exception(L"Key not found");
    ...
    json::value& operator[](const std::wstring& key)
    {
        auto iter = find_by_key(key);

        if (iter == m_elements.end() || key != (iter->first))
            return m_elements.insert(iter, std::pair<std::wstring, value>(key, value()))->second;
...
private:
    ...
    static bool compare_pairs(const std::pair<std::wstring, value>& p1, const std::pair<std::wstring, value>& p2)
    {
        return p1.first < p2.first;
    }
    static bool compare_with_key(const std::pair<std::wstring, value>& p1, const std::wstring& key)
    {
        return p1.first < key;
    }

    storage_type::const_iterator find_by_key(const std::wstring& key) const
    {
        if (m_keep_order)
        {
            return std::find_if(m_elements.begin(), m_elements.end(),
                [&key](const std::pair<std::wstring, value>& p) {
                    return p.first == key;
            });
        }
        else
    ...
    storage_type::iterator find_by_key(const std::wstring& key)
    {
        if (m_keep_order)
        {
            return std::find_if(m_elements.begin(), m_elements.end(),
    ...
    const json::value& at_internal(const std::wstring& key) const
    {
        auto iter = find_by_key(key);
    ...
    const_iterator find_internal(const std::wstring& key) const
    {
        auto iter = find_by_key(key);
    ...
    iterator find_internal(const std::wstring& key)
    {
        auto iter = find_by_key(key);
...
public:
    ...
    virtual bool has_field(const std::wstring &) const { return false; }
    virtual value get_field(const std::wstring &) const { throw json_exception(L"not an object"); }
    virtual value get_element(array::size_type) const { throw json_exception(L"not an array"); }

    virtual value &index(const std::wstring &) { throw json_exception(L"not an object"); }
    virtual value &index(array::size_type) { throw json_exception(L"not an array"); }

    virtual const value &cnst_index(const std::wstring &) const { throw json_exception(L"not an object"); }
    virtual const value &cnst_index(array::size_type) const { throw json_exception(L"not an array"); }
    ...
    virtual bool is_integer() const { throw json_exception(L"not a number"); }
    virtual bool is_double() const { throw json_exception(L"not a number"); }

    virtual json::number as_number() { throw json_exception(L"not a number"); }
    virtual double as_double() const { throw json_exception(L"not a number"); }
    virtual int as_integer() const { throw json_exception(L"not a number"); }
    virtual bool as_bool() const { throw json_exception(L"not a boolean"); }
    virtual json::array& as_array() { throw json_exception(L"not an array"); }
    virtual const json::array& as_array() const { throw json_exception(L"not an array"); }
    virtual json::object& as_object() { throw json_exception(L"not an object"); }
    virtual const json::object& as_object() const { throw json_exception(L"not an object"); }
    virtual std::wstring as_string() const { throw json_exception(L"not a string"); }
...
public:
    ...
    _String(const _String& other) : json::details::_Value(other)
    {
        copy_from(other);
    }
...
private:
    ...
    std::wstring as_utf16_string() const;
...
void format_string(const std::wstring& key, std::wstring& str);

void format_string(const std::wstring& key, std::string& str);
...
public:
    ...
    virtual json::value &index(const std::wstring &key);
...
/// <param name="key">The name of the field</param>
/// <returns>True if the field exists, false otherwise.</returns>
inline bool json::value::has_field(const std::wstring& key) const
{
...
/// <param name="key">The name of the field</param>
/// <returns>The value kept in the field; null if the field does not exist</returns>
inline json::value json::value::get(const std::wstring& key) const
{
...
std::wostream& operator << (std::wostream &os, const json::value &val);
...
std::wistream& operator >> (std::wistream &is, json::value &val);
