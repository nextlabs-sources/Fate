...
my	$maintenanceVer = 0;
my	$patchVer = 0;
my	$msiFilePrefix = "key-management-client";
...
my	$installDir = "$buildRootDir/install/KMService";
...
my	@bin32FileList = (
		["cesdk32.dll",						$versionStr, "x86", $releaseOnly],
		["ceservice32.dll",					$versionStr, "x86", $releaseOnly],
		["KeyManagementConsumer32.dll",		$versionStr, "x86", $releaseOnly],
		["KeyUtil.exe",						$versionStr, "x86", $releaseOnly],
		["nlQuench.exe",					$versionStr, "x86", $releaseOnly],
		["PluginInstallerSDK32.dll",		$versionStr, "x86", $releaseOnly]
	);
my	@bin64FileList = (
		["cesdk.dll",						$versionStr, "x64", $releaseOnly],
		["ceservice.dll",					$versionStr, "x64", $releaseOnly],
		["KeyManagementConsumer.dll",		$versionStr, "x64", $releaseOnly],
		["KeyUtil.exe",						$versionStr, "x64", $releaseOnly]
);
...
# Java
InstallHelper::copyRequired("$srcJavaDir/KeyManagementService.jar",		"$destJavaDir/KeyManagementService.jar");
...
my	$keyMgmtConfigDir = "$buildRootDir/prod/pc/key_management/KeyManagementConsumer/config";
...
# Java
InstallHelper::copyRequired("$keyMgmtConfigDir/KeyManagementService.properties",				"$destArtifactDir/KeyManagementService.properties");
...
InstallHelper::copyRequired("$destArtifactDir/KeyManagementService.properties",				"$toRes32Dir/KeyManagementService.properties");
...
#InstallHelper::copyRequired("$kmcConfigDir/product.xml",					"$package32Dir/product.xml");
#InstallHelper::copyRequired("$kmcConfigDir/product.xml",					"$package64Dir/product.xml");
