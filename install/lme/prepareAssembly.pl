...
#			c:\nightly\current\D_SiriusR2\install\lme\desktop-enforcer-5.5.0.0-10001-dev-20110321063014\
...
#			Compiling...
#			Setup.rul
#			c:\nightly\current\D_SiriusR2\install\lme\desktop-enforcer-5.5.0.0-10001-dev-20110321063014\script files\Setup.rul(90)
...
my	$maintenanceVer = 0;
my	$patchVer = 0;
my	$msiFilePrefix = "live-meeting-enforcer";
...
my	$installDir = "$buildRootDir/install/lme";
...
my	@bin32FileList = (
		["nllme32.dll",					$versionStr, "x86", $releaseOnly],
...
#
# Notes: LME does not need license file right now.

print "INFO: Copying support files\n";

my	$lmeProjDir = "$buildRootDir/prod/pep/endpoint/lme";
my	$lmeConfigDir = "$lmeProjDir/configuration";
my	$srcResDir = "$installDir/resource";
my	$destResDir = "$assemblyDir/resource";
my	$destRes32Dir = "$assemblyDir/resource32";

# Common
#InstallHelper::copyRequired("$lmeConfigDir/license.cfg",					"$destResDir/license.cfg");
InstallHelper::copyRequired("$lmeConfigDir/TamperResistance.cfg",			"$destResDir/LiveMeetingEnforcer_TamperResistance.cfg");
...
InstallHelper::copyRequired("$srcResDir/deleteLME.reg",								"$destResDir/deleteLME.reg");

# 32-bit
InstallHelper::copyRequired("$lmeConfigDir/PWConsole.exe.ini",				"$destRes32Dir/PWConsole.exe.ini");
...
#InstallHelper::copyRequired("$lmeConfigDir/product.xml",					"$package32Dir/product.xml");
