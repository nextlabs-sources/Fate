...
typedef enum _CEResult_t {
  ...
  CE_RESULT_RECURSIVE_INVOCATION                = -31, /**< Hooking recursion.  SDK function was called by a thread while executing another SDK function */
  CE_RESULT_TOO_MANY_REQUESTS                   = -32, /**< Number of requests > MAX_NUM_REQUESTS */
  CE_RESULT_THREAD_NOT_INITIALIZED              = -33, /**< Thread is not initialized*/
} CEResult_t;
...
/**
 * Named attributes (i.e. a named dimension or context)
 */
typedef struct _CENamedAttributes {
  CEString name;
  CEAttributes attrs;
} CENamedAttributes;
...
#define CE_NUM_RECIPIENTS_MAX           65536
...
extern "C" {
  ...
  // The largest number of requests that can be processed in one call to CEEVALUATE_CheckResourcesEx
#define CE_MAX_NUM_REQUESTS 250
   ...
   * numRequests (INPUT): the number of items in requests. This must be less than
   *  CE_MAX_NUM_REQUESTS (250)
...
#if defined (WIN32) || defined (_WIN64)
  CEResource *
  CEM_CreateResourceW (_In_ const wchar_t * resourceName, _In_opt_ const wchar_t * resourceType);
  CEResource *
  CEM_CreateResourceA (_In_ const char * resourceName, _In_opt_ const char * resourceType);
#ifdef UNICODE 
