...
public:
	...
	*/
	virtual void InitImage( CImage* _img, HINSTANCE _hInst,  UINT nIDResource ) 
	{
		//_img.LoadFromResource( _hInst, nIDResource ) ;
		GetResourceStream(_hInst, _img, MAKEINTRESOURCE( nIDResource ) , L"PNG" ) ;
	};
	virtual void ReleaseImage( CImage _img ) 
	{
		if( !_img.IsNull() )
		{
			_img.ReleaseDC() ;
	...
	virtual UINT GetResourceStream( HINSTANCE hInst,CImage* pImg, LPCWSTR lpName, LPCWSTR lpType ) 
	{
		UINT iRet = 0 ;
