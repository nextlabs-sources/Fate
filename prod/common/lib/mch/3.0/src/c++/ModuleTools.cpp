// ***************************************************************
//  ModuleTools.cpp           version: 1.0.0   date: 2010-01-10
//  -------------------------------------------------------------
...
SYSTEMS_API HMODULE WINAPI GetCallingModule(void)
{
      ...
      __try
      {
        ...
        for (int i = g_pHookCollection->GetCount() - 1; i >= 0; i--)
        {
          if ( ((*g_pHookCollection)[i].pCodeHook != NULL) &&
               ((*g_pHookCollection)[i].pCodeHook->mpInUseArray != NULL) &&
               (pReturnAddress >= (*g_pHookCollection)[i].pCodeHook->mpInUseArray) &&
               (pReturnAddress <  (*g_pHookCollection)[i].pCodeHook->mpInUseArray + inUseArraySize) )
          {
            ...
            pReturnAddress = ((IN_USE *) pReturnAddress)->returnAddress;
    ...
    if (result)
    {
      ...
      if (result)
      {
        *phModule = (HMODULE) mbi.AllocationBase;
        ...
        Trace(L"%S Failure - GetModuleFileName: %X", GetLastError());
    }
    else
      Trace(L"%S Failure - VirtualQuery");
  }
...
SYSTEMS_API IMAGE_NT_HEADERS* WINAPI GetImageNtHeaders(HMODULE hModule)
{
  ...
  __try
  {
    if (!IsBadReadPtr((CONST VOID *) hModule, 2))
    {
        ...
        *phModule = (HMODULE) mbi.AllocationBase;
        ...
        if (pNtHeaders != NULL)
        {
          ...
          *pImageExportDirectory = (IMAGE_EXPORT_DIRECTORY *) ((ULONG_PTR) *phModule + virtualAddress);
          result = (pImageExportDirectory != NULL);
        }
        else
          Trace(L"%S Failure - GetImageNtHeaders returned NULL", __FUNCTION__);
      }
...
SYSTEMS_API void WINAPI InitKernelProcs(void)
{
  ...
  if (!initialized)
  {
    ...
    if (hKernel != NULL)
    {
      ...
      pfnWaitForMultipleObjects = (PFN_WAIT_FOR_MULTIPLE_OBJECTS) KernelProc(CWaitForMultipleObjects, true);
      pfnWaitForSingleObject = (PFN_WAIT_FOR_SINGLE_OBJECT) KernelProc(CWaitForSingleObject, true);
      pfnSleep = (PFN_SLEEP) KernelProc(CSleep, true);
      pfnVirtualQuery = (PFN_VIRTUAL_QUERY) KernelProc(CVirtualQuery, true);
      pfnLocalAlloc = (PFN_LOCAL_ALLOC) KernelProc(CLocalAlloc, true);
      pfnLocalFree = (PFN_LOCAL_FREE) KernelProc(CLocalFree, true);
      pfnSetLastError = (PFN_SET_LAST_ERROR) KernelProc(CSetLastError, true);
      pfnReleaseMutex = (PFN_RELEASE_MUTEX) KernelProc(CReleaseMutex, true);
    }
  }
}