...
//Shouldn't distinguish enforcer type on policy controller, 
//return false in oder to be same as on Linux
bool getPDPType(int dwArgc, char* *lpszArgv)
{
  return false;
}
//Of course, we don't support WDE on Mac
bool CheckWDESupport(int dwArgc, char* *lpszArgv)
{
  return false;
}
//We don't support bootstrap injection service on Mac now
