...
bool CWinAD::SearchUserInfo(std::wstring& strEMail,std::wstring& strSID,const wchar_t* wstrkeyWord)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			wcsncpy_s(szPath,MAX_PATH/sizeof(OLECHAR),L"LDAP://",MAX_PATH);
			wcsncat_s(szPath,MAX_PATH/sizeof(OLECHAR),var.bstrVal,MAX_PATH-wcslen(szPath));
			...
			if (SUCCEEDED(hr))
			{
				...
				{
					if (S_FALSE==hr)
						OutputDebugStringW(L"User object cannot be found.\n");
				}
				else if (E_ADS_INVALID_FILTER==hr)
					OutputDebugStringW(L"Cannot execute query. Invalid filter was specified.\n");
				//else
				//OutputDebugStringW(L"Query failed to run. HRESULT: %x\n",hr);
			}
			else
			{
				OutputDebugStringW(L"Cannot execute query. Cannot bind to the container.\n");
			}
...
/* ************************************************** */
HRESULT CWinAD::FindUsers(IDirectorySearch *pContainerToSearch,  // IDirectorySearch pointer to the container to search.
