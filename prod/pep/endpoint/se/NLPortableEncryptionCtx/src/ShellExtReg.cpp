...
STDAPI RegisterFileMenu()
{
   AFX_MANAGE_STATE(AfxGetStaticModuleState( ));
	...
	StringFromIID(CLSID_MyFileNLPortableEncryptionCtxID, &pwszShellExt);			    
	...
	REGSTRUCT ShExClsidEntries[] = {  
		...
		HKEY_CLASSES_ROOT,	TEXT("CLSID\\%s"),					NULL,                   SHELLEXNAME,
	...
	for(i = 0; ShExClsidEntries[i].hRootKey; i++) 
   {
		...
		_snwprintf_s(szSubKey, MAX_PATH, _TRUNCATE, ShExClsidEntries[i].lpszSubKey, szCLSID);
	   ...
	   if(NOERROR == lResult) 
      {
			...
			_snwprintf_s(szData, MAX_PATH, _TRUNCATE, ShExClsidEntries[i].lpszData, szModule);
	...
	REGSTRUCT OtherShExEntries[] = {  
			...
			HKEY_CLASSES_ROOT,	L"*\\shellex\\ContextMenuHandlers\\"SHELLEXNAME, NULL,		TEXT("%s"),
   ...
   {
		//Create the sub key string.
		_snwprintf_s(szSubKey, MAX_PATH, _TRUNCATE, OtherShExEntries[i].lpszSubKey, szCLSID);
		...
		if(NOERROR == lResult) {
			...
			//if necessary, create the value string
			_snwprintf_s(szData, MAX_PATH, _TRUNCATE, OtherShExEntries[i].lpszData, szCLSID);
...
STDAPI UnregisterFileMenu(void)
{
	...
	REGSTRUCT OtherShExEntries[] = {  
			...
			// our CLSID is placed in the "%s", and so, explorer will take that CLSID, look in HKEY_CLASSES_ROOT/CLSID
			// to find a matching CLSID and use the info in there to find our DLL... whew. 
			HKEY_CLASSES_ROOT,	L"*\\shellex\\ContextMenuHandlers\\"SHELLEXNAME, NULL,		NULL,
	...
	// NT needs to have shell extensions "approved".
	if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId)  {
		lstrcpy( szSubKey, L"Software\\Microsoft\\Windows\\CurrentVersion\\Shell Extensions\\Approved\\"SHELLEXNAME);
