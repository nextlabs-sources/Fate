...
my	$maintenanceVer = 0;
my	$patchVer = 0;
my	$msiFilePrefix = "office-communicator-enforcer";
...
my	$installDir = "$buildRootDir/install/oce";
...
my	@bin32FileList = (
		["ipcproxy32.dll",				"5.6.0.0", "x86", $releaseOnly],
		["uccpPEP32.dll",				$versionStr, "x86", $releaseOnly],
		["nlQuench.exe",				"5.6.0.0", "x86", $releaseOnly],
		["PluginInstallerSDK32.dll",	"5.6.0.0", "x86", $releaseOnly]
	);
my	@bin64FileList = (["ipcproxy.dll",				"5.6.0.0", "x64", $releaseOnly]
);
...
#
# Notes: OCE does not need license file right now.

print "INFO: Copying support files\n";

my	$oceProjDir = "$buildRootDir/prod/pep/endpoint/oce";
my	$oceConfigDir = "$oceProjDir/configuration";
my	$srcResDir = "$installDir/resource";
my	$destResDir = "$assemblyDir/resource";
my	$destRes32Dir = "$assemblyDir/resource32";

# Common
#InstallHelper::copyRequired("$oceConfigDir/license.cfg",					"$destResDir/license.cfg");
InstallHelper::copyRequired("$oceConfigDir/TamperResistance.cfg",			"$destResDir/OfficeCommunicatorEnforcer_TamperResistance.cfg");
...
# 32-bit
InstallHelper::copyRequired("$oceConfigDir/communicator.exe.ini",			"$destRes32Dir/communicator.exe.ini");
...
InstallHelper::copyRequired("$oceConfigDir/product.xml",					"$package32Dir/product.xml");
InstallHelper::copyRequired("$oceConfigDir/product.xml",					"$package64Dir/product.xml");
