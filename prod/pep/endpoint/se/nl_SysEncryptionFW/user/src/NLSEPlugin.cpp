...
#define CELOG_CUR_MODULE L"NLSEFWPlugin"
...
//NLSysEncryption service should be started at boot time in order to
...
static bool StartNLSEService()
{
  ...
  if(ssStatus.dwCurrentState == SERVICE_STOPPED || 
    ...
    CELOG_LOG(CELOG_DEBUG,
	     L"NLSysEncryption service has been stopped.\n");
    CloseServiceHandle(schService); 
    CloseServiceHandle(schSCManager);
    return false; 
  }
  ...
  if (ssStatus.dwCurrentState == SERVICE_RUNNING) {
    CELOG_LOG(CELOG_INFO,
	     L"NLSysEncryption Service started.\n");
  } else { 
    CELOG_LOG(CELOG_ERROR,L"NLSysEncryption Service not started. \n");
...
extern "C" __declspec(dllexport) int PluginEntry( void** in_context )
{
  ...
  *in_context = (void*)context;
  ...
  if(!StartNLSEService()) {
    CELOG_LOG(CELOG_CRITICAL,L"nlse plugin cannot start NLSE service\n");
    goto PluginEntry_complete;
  }
