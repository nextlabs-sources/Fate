...
inline BOOL Deallocate( void *p )
{
   BOOL result = HeapFree( GetProcessHeap(), 0, p );
   return result;
}
...
inline int osMultiByteToWideChar( const char *mbSString, 
                                  int cbMultiByte, 
                                  wchar_t *wSString, 
                                  int cchWideChar )
{
...
inline int osWideCharToMultiByte( const wchar_t *wSString, 
                                  int cchWideChar, 
                                  char *mbSString, 
                                  int cbMultiByte )
{
...
class SYSTEMS_API SString
{
   public:
      ...
      ~SString( )
      {
         if( mDeleteBufferOnDestruct == TRUE )
         {
            Deallocate( mpData );
            mpData = NULL;
            mpEndPos = NULL;
      ...
      {
         return( 0 == wcscmp( s.mpData, t.mpData ) );
      }

      friend bool operator ==( const SString& s , const wchar_t *t )
      {
         return( 0 == wcscmp( s, t ) );
      }

      // Inequality operator. All comparisons are case sensitive.
      friend bool operator !=( const SString& s , const SString& t )
      {
         return( 0 != wcscmp( s.mpData, t.mpData ) );
      }
      ...
      SString& operator =( LPCWSTR SString )
      {
      ...
      SString& operator =( const char *SString )
      {
         // If SString is empty, fix up this.
         ...
         else
         {
            ...
            int d = osMultiByteToWideChar( SString, 
                                          -1, 
                                          mpData,
                                          (int)length + 1 );
            if( d == 0 )
      ...
      // cast Operator to allow us to use the SString in API functions.
      ...
      // This is a new buffer that must be freed with DeallocateBuffer
      operator char *() const
      {
         size_t length = Length()+1;
         char *buffer = (char *)Allocate( length * sizeof( char ) );
         if( osWideCharToMultiByte( mpData, -1, buffer, (int) length ) == (int)length )
            return buffer;
         else
         {
            Deallocate(buffer);
      ...
      friend SString operator +( const SString& s, const wchar_t *s1)
      {
	      if( NULL == s1 )
	      {
		      return( s );
      ...
      int Format( const wchar_t * szFmt , ... )
      {
         ...
         int iRet = vswprintf( mpData , szFmt , marker );
      ...
      // convenient if you are going to be adding a lot to this SString.
      ...
      bool Replace( const wchar_t *replaceThis, const wchar_t *withThis )
      {
	      if( mpData == NULL )
	      {
		      return false;
      ...
      // delimeter is character between each SString.
	   wchar_t *SubStr( int index, wchar_t delimeter = L'|' )
      {
      ...
      static int MultiByteToWideChar( const char *mbSString, int cbMultiByte, wchar_t *wSString, int cchWideChar )
      {
         return osMultiByteToWideChar( mbSString, cbMultiByte, wSString, cchWideChar );
      }

      static int WideCharToMultiByte( const wchar_t *wSString, int cchWideChar, char *mbSString, int cbMultiByte )
      {
   ...
   private:
      ...
      // property is not copied over when copying SStrings.  You must
      // specifically call DoNotDeleteBuffer or construct the SString
