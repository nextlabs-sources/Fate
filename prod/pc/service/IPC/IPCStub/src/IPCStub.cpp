...
bool IPCStub::Init(IIPCRequestHandler** pRequestHandlerArray, int threadPoolSize) 
{
    ...
    tstring serverSharedMemoryFileName = GLOBAL_NAME_PREFIX;
    serverSharedMemoryFileName += requestHandlerClassName;
    serverSharedMemoryFileName += SHARED_MEMORY_SUFFIX;
    ...
    m_hSharedFileMapping = CreateFileMapping(INVALID_HANDLE_VALUE,    // current file handle 
        SecurityAttributesFactory::GetSecurityAttributes(), // security attributes
        PAGE_READWRITE,                    // read/write permission 
        0,                                 // max. object size 
        CHANNEL_SIZE,                              // size of hFile 
        serverSharedMemoryFileName.c_str());            // name of mapping object 

    if (!m_hSharedFileMapping)
    {
        m_hSharedFileMapping = ::OpenFileMapping (FILE_MAP_WRITE, // read/write permission 
            FALSE,               // Do not inherit the name 
            serverSharedMemoryFileName.c_str());             // of the mapping object. 
    }
    m_pSharedMemory = (TCHAR*) ::MapViewOfFile(m_hSharedFileMapping, // handle to mapping object 
    ...
    m_threadArray = new HANDLE [threadPoolSize];
    m_requestDispatcherArray = new RequestDispatcher* [threadPoolSize];
...
void IPCStub::RemoveChannel (HANDLE hReceiveEvent)
{
    if (m_pLog->IsInfoEnabled ())
    {
        ...
        _stprintf (log, _T("Channel removed: %d\n"), hReceiveEvent);
        m_pLog->Trace(log);
    }

    ::WaitForSingleObject (m_hEventListMutex, INFINITE);

    PtrToPtrMap::iterator iter = m_eventChannelMap.find((void*)(hReceiveEvent));
    IPCMessageChannel* pMessageChannel = (IPCMessageChannel*) iter->first;
...
void IPCStub::Run(intptr_t eventBucketNumber)
{
    ...
    while (!m_bStopped)
    {
        ...
        {
            TCHAR log[200];
            _stprintf (log, _T("WaitForMultipleObjects call failed: Error# %d\n"), ::GetLastError ());
            m_pLog->Error (log);
        }
        ...
        else if (event - WAIT_OBJECT_0 < static_cast<DWORD>(numEvents) &&
        ...
        {
            TCHAR log[200];
            _stprintf(log, _T("WaitForMultipleObjects call returned unknown return value %d\n"), event);
            m_pLog->Error(log);
        }
...
void IPCStub::PerformHandshake ()
{
    ...
    {
        TCHAR log[CHANNEL_SIZE + 100];
        _stprintf (log, _T("Handshake Request Received: \n%s\n"), buf);
        m_pLog->Info (log);
    }
    ...
    if (_tcscmp (pToken, CONNECT) == 0)
    {
        ...
        if (!pToken)
        {
            m_pLog->Error (_T("Invalid handshake request..."));
        }
