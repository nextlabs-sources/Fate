...
enum PGPtlsLoggingLevel_
{
	kPGPtls_LogLevel_None			= 0,
	
	kPGPtls_LogLevel_Error			= 1,
	kPGPtls_LogLevel_Warning		= 2,
	kPGPtls_LogLevel_Info			= 3,
	kPGPtls_LogLevel_Config			= 4,
	kPGPtls_LogLevel_Fine			= 5,

	PGP_ENUM_FORCE( PGPtlsLoggingLevel_ )
};
PGPENUM_TYPEDEF( PGPtlsLoggingLevel_, PGPtlsLoggingLevel );


typedef void (*PGPtlsLoggingCallback)( void *userLoggingData, PGPtlsLoggingLevel msgLoggingLevel, const char *msg );
...
/*____________________________________________________________________________
	...
	In the general case, the remoteID will be an IP address + port.  This
...
/*____________________________________________________________________________
	Sets up the logging callback 'loggingCallback' that will be called for any
	message with logging level above or equal to 'maxLoggingLevel'.
	The parameter passed to 'loggingCallback' will indicate exact logging
	level for the message.
	Calling with kPGPtls_LogLevel_None, the default setting, disables any logging
	and is the most performance-efficient setting.
	Each invocation of the callback receives the the message that can be
	displayed to a user, although it may be meaningless without few previous 
	or following messages. The message can be a multiline message,
	such as the X.509 PEM certificate.
____________________________________________________________________________*/
PGPError	PGPtlsSetupLogging( PGPtlsSessionRef ref, PGPtlsLoggingLevel maxLoggingLevel,
				PGPtlsLoggingCallback loggingCallback, void *userLoggingData );
