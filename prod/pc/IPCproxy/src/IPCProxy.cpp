...
bool IPCProxy::Init (const TCHAR* requestHandlerClassName)
{
    ...
    receiveEventName = new (std::nothrow) TCHAR[nameLength + _tcslen(RECEIVE_EVENT_SUFFIX) + 1];
    mutexName = new (std::nothrow) TCHAR[nameLength + _tcslen(MUTEX_SUFFIX) + 1];
    if ((receiveEventName == NULL) || (mutexName == NULL))
    {
        bRetVal = false;
        goto label_fail;
    }
    ...
    bool ret = GetSharedMemMapping();
	if (ret != true)
	{
        TCHAR buf[256];
        _sntprintf_s(buf, 256, _TRUNCATE, _T("Unable to get shared memory mapping (err = %d)\n"), ::GetLastError());
        m_pLog->Error(buf);
        bRetVal = false;
        goto label_fail;
	}
    ...
    if (m_hSendEvent == 0) {
        TCHAR buf[256];
        _sntprintf_s(buf, 256, _TRUNCATE, _T("Unable to open event %s (err = %d)\n"), sendEventName, ::GetLastError());
        m_pLog->Error(buf);
        bRetVal = false;
        goto label_fail;
    }
    ...
    if (m_hMutex == 0) {
        TCHAR buf[256];
        _sntprintf_s(buf, 256, _TRUNCATE, _T("Unable to open mutex %s (err = %d)\n"), mutexName, ::GetLastError());
        m_pLog->Error(buf);
        bRetVal = false;
        goto label_fail;
    }
    ...
    if (m_pLog->IsInfoEnabled ())
    {
        ...
        _sntprintf_s (log, CHANNEL_SIZE + 100, _TRUNCATE, _T("Handshake Request Sent: %s\n"), handshakeSharedMem);
        m_pLog->Info (log);
        delete []log;
    }
    ...
    if (::WaitForSingleObject (hReceiveEvent, SERVER_RESPONSE_TIMEOUT) == WAIT_TIMEOUT) {
        ::ReleaseMutex(hMutex);
        TCHAR *log = new TCHAR[CHANNEL_SIZE + 100];
        _sntprintf_s(log, CHANNEL_SIZE + 100, _TRUNCATE, _T("Never got response for handshake request: %s\n"), handshakeSharedMem);
        m_pLog->Error(log);
        delete []log;
        bRetVal = false;
        goto label_fail;
...
bool IPCProxy::Invoke (IPCREQUEST* pRequest, IPCREQUEST* pResponse, UINT timeout)
{
    ...
    {
        TCHAR log[100];
        _sntprintf_s (log, 100, _TRUNCATE, _T("Thread %d: Received Response: ?\n"), ::GetCurrentThreadId());
        m_pLog->Info (log);
    }
...
void IPCProxy::Uninit()
{
    if (m_bInitialized) 
    {
        ...
        if (::WaitForSingleObject (m_hMutex, UNINIT_TIMEOUT) != WAIT_TIMEOUT)
        {
            ...
            _tcsncpy_s (pRequest->methodName, 64, DISCONNECT, _TRUNCATE);
            ::SetEvent (m_hSendEvent);
