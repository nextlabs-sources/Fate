	...
	m_pLibHandler(NULL)
{
	...
	if (IsPsapiSupported())
        ...
        // FIXIT -- there is no need to support toolhelp at all
...
CPsapiHandler::CPsapiHandler(CRunningProcesses* pProcesses):
    ...
    m_pfnGetProcessImageFileNameA (NULL)
{
    ...
    if (NULL != m_hModPSAPI)
	{
		...
		m_pfnGetModuleFileNameExA = (PFNGETMODULEFILENAMEEXA)
			::GetProcAddress(m_hModPSAPI, "GetModuleFileNameExA");
                ...
                m_pfnGetProcessImageFileNameA = (PFNGETPROCESSIMAGEFILENAMEA)
			::GetProcAddress(m_hModPSAPI, "GetProcessImageFileNameA");
...
BOOL CPsapiHandler::PopulateProcess(DWORD dwProcessId, BOOL bPopulateModules)
{
    ...
    if (TRUE == Initialize())
    {
        ...
        if (hProcess) 
        {
            //FIXME this code block is really wierd. Need rewrite after 1.0
            int nCountLoop = 0; // FIX IT -- we need to introduce
            // some reasonable time out value
            while ((m_pfnEnumProcessModules(hProcess, hModuleArray,sizeof(hModuleArray), &cbNeeded) != TRUE) ||
                (cbNeeded == 0))
            {
            ...
            if (cbNeeded == 0)
            {
                char    szModuleName[MAX_PATH];
				szModuleName [0] = 0;
                //m_pfnGetModuleFileNameExA (hProcess, NULL, szModuleName, sizeof (szModuleName));
                if (m_pfnGetProcessImageFileNameA)
                {
                    m_pfnGetProcessImageFileNameA (hProcess, szModuleName, sizeof (szModuleName));
				}
                pProcessInfo = new CExeModuleInstance(
                    this,
                    szModuleName, 
...
#ifdef _UNICODE
        AnsiToUnicode (pProcess->GetBaseName(), szProcessName, MAX_PATH*2);
#else
        strncpy_s (szProcessName, MAX_PATH*2, pProcess->GetBaseName(), _TRUNCATE);
#endif
		if (0 != _tcsicmp(szProcessName, me.szModule))  
		{
            char szExePath[MAX_PATH*2];
#ifdef _UNICODE
            UnicodeToAnsi (me.szExePath, szExePath, MAX_PATH*2);
#else
            strncpy_s (szExePath, MAX_PATH*2, me.szExePath, _TRUNCATE);
#endif
			pDllModuleInstance = new CModuleInstance(szExePath, me.hModule);
			pProcess->AddModule(pDllModuleInstance);
		}
		else
		//
...
BOOL CToolhelpHandler::PopulateProcess(DWORD dwProcessId, BOOL bPopulateModules)
{
	...
	if (TRUE == Initialize())
	{
        ...
        char szExeFile[MAX_PATH*2];
#ifdef _UNICODE
        UnicodeToAnsi (pe32.szExeFile, szExeFile, MAX_PATH*2);
#else
        strncpy_s (szExeFile, MAX_PATH*2, pe32.szExeFile, _TRUNCATE);
#endif
			pProcessInfo = new CExeModuleInstance(
				this,
				szExeFile, 
...
void CModuleInstance::Set_Name(char *pszName)
{
	...
	if ((NULL != pszName) && (strlen(pszName)))
	{
		...
		strncpy_s(m_pszName, strlen(pszName) + 1, pszName, _TRUNCATE);
	}
	else
	{
		m_pszName = new char[strlen("\0") + 1];
		strncpy_s(m_pszName, strlen("\0") + 1, "\0", _TRUNCATE);
	}
