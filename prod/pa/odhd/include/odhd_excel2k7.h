#ifndef __H_ODHD_EXCEL2K7__
#define  __H_ODHD_EXCEL2K7__
...
static StaticCatTitle gs_CatTitle_Excel2K7[]={
	{
...
static StaticItemResult gs_ItemTitle_Excel2K7[] = 
{
	...
	{L"Information in worksheet headers",L"",L"No information in worksheet headers.",L"Can't remove information in worksheet headers.",0},
	{L"Information in worksheet footers",L"",L"No information in worksheet footers.",L"Can't remove information in worksheet footers.",0},
	{L"Hidden rows",L"",L"No hidden rows found.",L"Can't remove hidden rows.",1},
	{L"Hidden columns ",L"",L"No hidden columns found.",L"Can't remove hidden columns",1},
	{L"Hidden worksheets",L"",L"No hidden worksheets found.",L"Cant' remove hidden worksheets.",1},
	{L"Custom XML data ",L"",L"No custom XML data found.",L"Can't remove custom XML data.",0},
	{L"Invisible Objects",L"",L"No invisible object found.",L"Can't remove invisible objects.",0},
	{L"AutoFilter data",L"AutoFilter data was found.",L"No AutoFilter data found.",L"Can't remove AutoFilter data.",0}
};


class CODHDExcel2K7Inspector:public CODHDInspector
{
public:
	CODHDExcel2K7Inspector():CODHDInspector((long)ODHDCAT_EXCEL_MAX,gs_CatTitle_Excel2K7)
		  ,m_pExcelApp(NULL)
		  ,m_pWorkbook(NULL)
		  ,m_bReadonly(FALSE)
		  ,m_bSaveAsed(FALSE)
	...
	virtual ~CODHDExcel2K7Inspector()
	{
		CComVariant varResult;
		if(m_pExcelApp != NULL)
...
public:
	...
	bool InspectInvisibleContentbyShapes(bool bRemove,int &nInvisibleContentNum);
	ODHDSTATUS	InspectInvisibleContentByInspector(Office::DocumentInspectors* spInspectors,ODHDCATEGORY odhdCat,ODHDITEM odhdItem,DWORD *dwInspectFail = NULL);
	BOOL	RemoveInvisibleContentByInspector(Office::DocumentInspectors* spInspectors,int iInspectorIndex,ODHDCATEGORY odhdCat,ODHDITEM odhdItem);
	ODHDSTATUS InspectInvisibleContentByInspector(Office::DocumentInspectors* spInspectors,int iInspectorIndex,ODHDCATEGORY odhdCat,ODHDITEM odhdItem,DWORD *dwInspectFail = NULL);
...
private:
	...
	DWORD	m_InspectorIndex2;//Headers and Footers
	DWORD	m_InspectorIndex3;//Hidden Rows and Columns
	DWORD	m_InspectorIndex4;//Hidden Wordsheets
	DWORD	m_InspectorIndex5;//Invisible Content
...
private:
	...
	BOOL HasScenarioComments(Excel::_Workbook* pWorkbook);
	BOOL HasPublishInformation(Excel::_Workbook* spWbk);
	BOOL HasConnections(Excel::_Workbook *spWbk);
	// remove properties
	...
	BOOL IsXLTM(LPCWSTR pwzFile)
	{
		LPCWSTR pSuffix = wcsrchr(pwzFile, L'.');
		if(NULL == pSuffix) return FALSE;
		if(0 == _wcsicmp(pSuffix, L".XLTM"))
			return TRUE;
		return FALSE;
	}
	BOOL IsXLTX(LPCWSTR pwzFile)
	{
		LPCWSTR pSuffix=wcsrchr(pwzFile,L'.');
		if(NULL == pSuffix) return FALSE;
		if(0 == _wcsicmp(pSuffix, L".XLTX"))
			return TRUE;
		return FALSE;
	}
	BOOL IsXLT(LPCWSTR pwzFile)
	{
		LPCWSTR pSuffix=wcsrchr(pwzFile,L'.');
		if(NULL == pSuffix) return FALSE;
	...
	long GetFileFormatForTemplate(LPCWSTR pwzFile)
	{
		if(IsXLTM(pwzFile))
			return xlOpenXMLTemplateMacroEnabled;
		if(IsXLTX(pwzFile))
			return xlOpenXMLTemplate;
		if(IsXLT(pwzFile))
			return xlTemplate8;
		return xlWorkbookDefault;
	}
	...
	BOOL SafeExcelDoc()
	{
		...
		if(m_bReadonly||CODHDUtilities::IsExcelTemplateFile(m_strTempFileName.c_str()))
		{
			...
			hr = AutoWrap(DISPATCH_PROPERTYGET,&varResult,m_pWorkbook,L"FileFormat",0);
			...
			CComVariant varFormat=covOptional;

#if WSO2013
			hr = AutoWrap(DISPATCH_PROPERTYPUT,&varResult,m_pWorkbook,L"CheckCompatibility",1,varFalse);
			hr = AutoWrap(DISPATCH_PROPERTYPUT,&varResult,m_pExcelApp,L"DisplayAlerts",1,varFalse);
#endif

			if(CODHDUtilities::IsExcelTemplateFile(m_strTempFileName.c_str()))
				varFormat=GetFileFormatForTemplate(m_strTempFileName.c_str());
	...
	bool IsWin10AndTemplateFile()
	{
		if (IsWindows10OrGreater()&&CODHDUtilities::IsExcelTemplateFile(m_strTempFileName.c_str()))
		{
			return true;
		}
		return false;
