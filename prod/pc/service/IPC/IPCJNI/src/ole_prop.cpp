	...
	case VT_BSTR: // OLE Automation string.
		{
			...
			char dbcs[1024];
			size_t i = 0;
			wcstombs_s(&i, dbcs, 1024, pPropVar->bstrVal, _TRUNCATE);
			printf("%s (VT_BSTR)\n", dbcs);
		}
         break;
...
void DumpCustomProps(IPropertySetStorage *pPropSetStg) 
{
...

	{
        ...
        propvarWrite.pszVal = "Property Value";

		hr=pPropStg->WriteMultiple(1,&propspec,&propvarWrite,PID_FIRST_USABLE );
		if(!FAILED(hr))
			printf("Write Property:ADD by Jim succeeded\n");
	}
	...
	while(pEnumProp->Next(1, &sps, &fetched) == S_OK) 
	{
		...
		// Read this property.
         ...
         if(!FAILED(hr)) 
		 {
			...
			char dbcs[1024];
			size_t i = 0;
			wcstombs_s(&i, dbcs, 1024, sps.lpwstrName, _TRUNCATE);
...
void DumpProps(TCHAR *wcFilename) 
{
	...
	if(FAILED(hr)) 
	{
		if(hr == STG_E_FILENOTFOUND)
			...
			printf("StgOpenStorage() failed w/error %08lx\n", hr);
		return;
	}
