...
#ifdef _DEBUG
            const std::wstring& strSID = SidToStringSid(pSID);
...
_exit:
    ...
    public:
        ...
        BOOL SetFileDaclAttributes(LPCWSTR pwzFileName)
        {
            ...
            //if(ERROR_SUCCESS==SetNamedSecurityInfoW((LPWSTR)pwzFileName, SE_FILE_OBJECT, secInfo, NULL, NULL, pAcl, NULL))
            //{
            //    return TRUE;
            //}
            //else
            //{
            //    DP((L"SetNamedSecurityInfoW fail (%d)\n", GetLastError()));
            //    return FALSE;
            //}
        }
    private:
