...
BOOL CNL_NotificationsList::InsertItemEx( LPARAM lParam, int nIndex)
{
	...
	int seed = timenow.wYear + timenow.wMonth + timenow.wDay + timenow.wHour + timenow.wMinute + 
				timenow.wSecond + timenow.wMilliseconds + item.iItem + item.iSubItem;

	srand(seed);//Generate a random number to be as a key
	int nKey = rand();
...
void CNL_NotificationsList::ShowDetails(int nIndex)
{
	int nSel = nIndex;
