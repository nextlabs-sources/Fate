...
const TCHAR *strWndClassName = _T("NLShadowWnd");
...
void CWndShadow::Create(HWND hParentWnd)
{
	...
	//	To write code that is compatible with both 32-bit and 64-bit versions of Microsoft Windows, use the SetWindowLongPtr function.
...
void CWndShadow::Update(HWND hParent)
{
	...
	BLENDFUNCTION blendPixelFunction= { AC_SRC_OVER, 0, 125, AC_SRC_ALPHA };

	MoveWindow(m_hWnd, ptDst.x, ptDst.y, nShadWndWid, nShadWndHei, FALSE);

	BOOL bRet= s_UpdateLayeredWindow(m_hWnd, NULL, &ptDst, &WndSize, hMemDC, &ptSrc, 0, &blendPixelFunction, ULW_ALPHA);
	bRet;
...
//	HRGN hParentRgn = CreateRectRgn(0, 0, rcParent->right - rcParent->left, rcParent->bottom - rcParent->top);
	HRGN hParentRgn = CreateRoundRectRgn(0, 0, rcParent->right - rcParent->left, rcParent->bottom - rcParent->top, m_nRoundWidth, m_nRoundHeight);
...
// 	HRGN hParentRgn = CreateRectRgn(0, 0, rcParent->right - rcParent->left, rcParent->bottom - rcParent->top);
	...
	int (*ptAnchorsOri)[2] = new (std::nothrow) int[szParent.cy][2];	// anchor points, will not modify during erosion
...
// 			_RPTF2(_CRT_WARN, "%d %d\n", ptAnchorsOri[i][0], ptAnchorsOri[i][1]);
	...
	int (*ptAnchorsTmp)[2] = new (std::nothrow) int[nAnchors + 2][2];	// Store the result of erosion
	...
	UINT32 *pKernel = new (std::nothrow) UINT32[(2 * nKernelSize + 1) * (2 * nKernelSize + 1)];
