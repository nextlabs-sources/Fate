...
public:
...

    {
        ...
        spVScroll = YLIB::smart_ptr<YVScrollBar>(new YVScrollBar(hParent));

        spHScroll = YLIB::smart_ptr<YHScrollBar>(new YHScrollBar(hParent));
    ...
    YLIB::smart_ptr<YVScrollBar> spVScroll;

    YLIB::smart_ptr<YHScrollBar> spHScroll;
...
protected:
...

    {
        ...
        if(m_offscreenSize.cy < wndRect.m_cy)

        {

            YLIB::WindowDC wndDC(m_hWnd);

            OffsetRect(&(wndRect.m_rect), 2, 2);

            if(NULL==m_hbWhite) m_hbWhite = CreateSolidBrush(RGB(255, 255, 255));

            if(NULL==m_hbWhite) return 0L;

            FillRect(wndDC.get_DC(), &(wndRect.m_rect), m_hbWhite);
    ...
    //    HDC         hViewDC = 0;
        ...
        //if(NULL==hb) return 0L;

        //FillRect(hDC, &(wndRect.m_rect), hb);
...
//  int  nWndHeight = wndRect.m_cy;
     //   HANDLE hThread = NULL;
        ...
        case SB_PAGEDOWN:
            ...
            if(m_viewOffset.cy>=m_offscreenSize.cy-wndRect.m_cy)
            {
                m_viewOffset.cy=(m_offscreenSize.cy-wndRect.m_cy);//+1);
            }
            break;
