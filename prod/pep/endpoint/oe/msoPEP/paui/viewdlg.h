...
public:
	...
	BEGIN_MSG_MAP(CViewDlg)
        ...
        MESSAGE_HANDLER(WM_ITEMHIDESHOW, OnItemHideShow)
...
public:
    ...
    BOOL AddItem(CDmData& dmData)
    {
        //smart_ptr<CViewItemDlg> spViewItem(new CDmItemDlg(dmData));
        CViewItemDlg* spViewItem = new CDmItemDlg(dmData);
    ...
    BOOL AddItem(CTagData& tagData)
    {
        //smart_ptr<CViewItemDlg> spViewItem(new CTagItemDlg(tagData));
        CViewItemDlg* spViewItem = new CTagItemDlg(tagData);
        m_vecItem.push_back(spViewItem);
        int i = (int)m_vecItem.size() - 1;
        m_vecItem[i]->m_nIndex = i;
        m_vecItem[i]->Create(m_hWnd, 0);
    ...
    BOOL AddItem(CInterData& interData)
    {
        //smart_ptr<CViewItemDlg> spViewItem(new CInterItemDlg(interData));
        CViewItemDlg* spViewItem = new CInterItemDlg(interData);
        m_vecItem.push_back(spViewItem);
        int i = (int)m_vecItem.size() - 1;
        m_vecItem[i]->m_nIndex = i;
        m_vecItem[i]->Create(m_hWnd, 0);
    ...
    BOOL AddItem(CMapData& mapData)
    {
        //smart_ptr<CViewItemDlg> spViewItem(new CDmItemDlg(dmData));
        CViewItemDlg* spViewItem = new CMapItemDlg(mapData);
        m_vecItem.push_back(spViewItem);
        int i = (int)m_vecItem.size() - 1;
        m_vecItem[i]->m_nIndex = i;
        m_vecItem[i]->Create(m_hWnd, 0);
...
protected:
	...
	LRESULT OnItemHideShow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        UNREFERENCED_PARAMETER(uMsg);
        UNREFERENCED_PARAMETER(wParam);
        UNREFERENCED_PARAMETER(lParam);
        ...
        for (VIEWITEMVECTOR::iterator it=m_vecItem.begin(); it!=m_vecItem.end(); ++it)
        {
            int nItemHeight = (*it)->GetRealHeight();
            _snprintf_s(szMsg, 256, _TRUNCATE, "Item(%02d) height = %d", (*it)->m_nIndex, nItemHeight);
            (*it)->SetWindowPos(HWND_TOP, CViewItemDlg::m_nX, m_nViewSize.cy, CViewItemDlg::m_nWidth, nItemHeight, SWP_SHOWWINDOW);
            m_nViewSize.cy += nItemHeight;
        }
        ResetView();
    ...
    LRESULT OnItemCheck(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        UNREFERENCED_PARAMETER(bHandled);
        HWND hParent = ::GetParent(m_hWnd);
        ::SendMessage(hParent, WM_ITEMCHECK, wParam, lParam);
        return 0L;
    }
    LRESULT OnItemCbnSelect(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        UNREFERENCED_PARAMETER(bHandled);
        HWND hParent = ::GetParent(m_hWnd);
        ::SendMessage(hParent, WM_ITEMCBNSELECT, wParam, lParam);
        return 0L;
    }
