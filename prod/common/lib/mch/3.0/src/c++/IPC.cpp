// ***************************************************************
//  IPC.cpp                   version: 1.0.0   date: 2010-01-10
//  -------------------------------------------------------------
...
SYSTEMS_API BOOL WINAPI AddAccessForEveryone(HANDLE processOrService, DWORD access)
{
  ...
  if ((pGetSecurityInfo != NULL) && (pSetSecurityInfo != NULL) && (pSetEntriesInACL != NULL))
  {
    ...
    if (type != SE_UNKNOWN_OBJECT_TYPE)
    {
      ...
      sidIdAuth.Value[4] = 0;
      sidIdAuth.Value[5] = 1;
      if (AllocateAndInitializeSid(&sidIdAuth, 1, 0, 0, 0, 0, 0, 0, 0, 0, &pSid))
      {
        ZeroMemory(&explicitAccess, sizeof(explicitAccess));
        explicitAccess.grfAccessPermissions = access;
        explicitAccess.grfAccessMode = GRANT_ACCESS;
        explicitAccess.Trustee.TrusteeForm = TRUSTEE_IS_SID;
        explicitAccess.Trustee.TrusteeType = TRUSTEE_IS_USER;

        // form = TRUSTEE_IS_SID -> ptstrName = SID
        explicitAccess.Trustee.ptstrName = (LPWSTR) pSid;

        if (pSetEntriesInACL(1, &explicitAccess, oldDacl, &newDacl) == 0)
        {
                                       ...
                                       BOOL handleMessages)
{
  ...
  if (UseLpcTransport())
  {
    ...
    if (answerInitialized)
    {
      ...
      result = (lpcMessagePrv.ProcessId != NULL);
      if (result && (answerLen != 0))
        hProcess = OpenProcess(SYNCHRONIZE, FALSE, lpcMessagePrv.ProcessId);
      else
...
int WINAPI LpcWorkerThread(LPC_WORKER_THREAD *workerThread)
{
  ...
  while (true)
  {
    if ((WaitForSingleObject(workerThread->Event, INFINITE) != WAIT_OBJECT_0) || (workerThread->Message == NULL))
      break;
  ...
  while (true)
  {
    ...
    for (int i = workerThreadCount - 1; i >= 0; i--)
    {
         ...
         (GetTickCount() - lpcQueue->WorkerThreads->operator [](i)->LastActive > 100))
      {
        ...
        SetEvent(lpcQueue->WorkerThreads->operator [](i)->Event);
        WaitForSingleObject(lpcQueue->WorkerThreads->operator [](i)->Handle, 100);
        CloseHandle(lpcQueue->WorkerThreads->operator [](i)->Event);
        CloseHandle(lpcQueue->WorkerThreads->operator [](i)->Handle);  // shut thread down
        P_LPC_WORKER_THREAD tmp = lpcQueue->WorkerThreads->operator [](i);
        lpcQueue->WorkerThreads->operator [](i) = NULL;
...
static int WINAPI LpcPortThread(LPC_QUEUE *lpcQueue)
{
  ...
  while (true)
  {
    ...
    if (!CheckNtFunction(CNtReplyWaitReceivePort, pNtReplyWaitReceivePort(lpcQueue->Port, pulUnknown, 
                         pLpcMsgOut, (P_LPC_MESSAGE) &(pMsg->ActualMessageLength))))
    {
      CloseHandle(hPort);
      LocalFree(pMsg);
         continue;
    ...
    // make sure it's an LPC_CONNECTION_REQUEST.  If it's LPC_PORT_CLOSED, then no message will be logged but the check function returns
    // false.  If it's anything else, a message is logged and the check returns false
    if (!CheckLpcMessageType(pMsg->MessageType))
    {
      LocalFree(pMsg);
      continue;
    }
    ...
    Private->ProcessId = GetCurrentProcessId();
      ...
      if (pLpcQueue->WorkerThreads != NULL)
      {
        for (i = 0; i < pLpcQueue->WorkerThreads->GetCount(); i++)
        {
          if ((*pLpcQueue->WorkerThreads)[i] != NULL)
          {
            ...
            WaitForSingleObject(workerThread->Handle, 100);
...
static bool InitIpcAnswer(bool create, LPCSTR name, DWORD counter, DWORD processId, IPC_ANSWER *answer, DWORD session)
{
  ...
  if (answer->Length > 0)
  {
    ...
    if (processId != 0)
      sprintf_s(nameBuf, MAX_PATH, "%s%s%d$%x", name, answerBufName, counter, processId);
    else
      sprintf_s(nameBuf, MAX_PATH, "%s%s%d", name, answerBufName, counter);
    ...
    else
    {
      ...
      if (answer->Map == NULL)
      {
        ...
        if (processId != 0)
          sprintf_s(nameBuf, MAX_PATH, "%s%d\\%s%s%d$%x", sessionName, session, name, answerBufName, counter, processId);
        else
          sprintf_s(nameBuf, MAX_PATH, "%s%d\\%s%s%d", sessionName, session, name, answerBufName, counter);
...
static DWORD GetLpcCounter(void)
{
  if (LpcCounterBuf == NULL)
  {
    ...
    HANDLE map = CreateFileMapping((HANDLE) -1, NULL, PAGE_READWRITE, 0, 4, fileMapName);
        ...
        if (hMap != NULL)
        {
          if (GetLastError() != ERROR_ALREADY_EXISTS)
          {
            ...
            if (pPipedIpcRec != NULL)
            {
              ...
              InitSecurityAttributes(&securityAttributes, &securityDescriptor, false);
              CreatePipe(&pPipedIpcRec->ReadPipeHandle, &pPipedIpcRec->WritePipeHandle, &securityAttributes, 0);
