...
BOOL CNL_ListView::EnableHeader(BOOL bEnable)
{
	return ::EnableWindow(GetHeader(), bEnable);
}

void CNL_ListView::SetSelectedItem(int nIndex)
{
	ListView_SetItemState(m_hWnd, nIndex, LVIS_SELECTED , LVIS_SELECTED );
}
...
void CNL_ListView::SetHeaderFont(HFONT hFont)
{
	SendMessage(GetHeader(), WM_SETFONT, (WPARAM)hFont, TRUE);
}
