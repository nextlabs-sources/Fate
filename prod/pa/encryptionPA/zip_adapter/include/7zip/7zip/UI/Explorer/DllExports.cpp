...
static BOOL RegisterServer(CLSID clsid, LPCWSTR title)
{
  ...
  for(int i = 0; clsidEntries[i].hRootKey; i++)
  {
    ...
    _snwprintf_s(subKey, MAX_PATH, _TRUNCATE, clsidEntries[i].SubKey, clsidString);
    NRegistry::CKey key;
...
static BOOL UnregisterServer(CLSID clsid)
{
  ...
  TCHAR subKey[MAX_PATH];
  _snwprintf_s(subKey, MAX_PATH, _TRUNCATE, kClsidInprocMask, clsidString);
  RegDeleteKey(HKEY_CLASSES_ROOT, subKey);
  
  _snwprintf_s (subKey, MAX_PATH, _TRUNCATE, kClsidMask, clsidString);
  RegDeleteKey(HKEY_CLASSES_ROOT, subKey);
