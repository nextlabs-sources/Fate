...
#pragma warning( pop )
	if (strlen(pszParam) > 0)
	{
		...
		if (pdest)
			*pnCommaPos = (long)(pdest - pszParam - 1); //convert position information to long
		else
			*pnCommaPos = (int)strlen(pszParam); //convert size_t to integer
		memcpy(
			(PBYTE)pszResult, 
			(PBYTE)pszParam, 
			((*pnCommaPos) + 1)
     ...
     int    cchWideChar)
{
    if (pszAnsiBuff)
    {
        int iRet = MultiByteToWideChar(
           ...
           (int)strlen(pszAnsiBuff) + 1, // number of bytes in string //convert size_t to integer
...
static TCHAR* getFileAccessString(DWORD access) {
	...
	_tcsncpy_s(cResult, result.length() + 1, result.c_str(), _TRUNCATE);
	return cResult;
}
    ...
    while (start != NULL) {
        ...
        *end = L'\0';
        ...
        if ((wcslen(dir) + wcslen(start)) < MAX_PATH) {
            ...
            //OutputDebugString(L"ParseDialogFileList: file name too long, ignored");
        }
