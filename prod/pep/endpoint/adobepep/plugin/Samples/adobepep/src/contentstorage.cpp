...
bool CContextStorage::StoreDragDropContentFileInfo(const std::string& filePath)
{
    return Set(kDragDropContent, filePath);
}

bool CContextStorage::GetDragDropContentFileInfo(std::string& filePath)
{
    if (!Get(kDragDropContent, filePath))
    {
        return false;
    }
