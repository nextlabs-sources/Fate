...
public:
    ...
    BOOL PrepareView()
    {
        assert(NULL!=m_hViewWnd);
        int nOffset = 0;
        for(std::vector<smart_ptr<TCell>>::iterator it=m_cells.begin();
            it!=m_cells.end(); ++it)
        {
            int cx=0, cy=0;
            smart_ptr<TCell> spCell = *it;
            if(0==spCell.get()) continue;
            cy = spCell->prepare_Cell(nOffset);
            cx = spCell->get_Width();
            if(cx>0 && cy>0)
            {
                m_offscreenSize.cx = max(m_offscreenSize.cx, cx);
                nOffset += cy;
                m_offscreenSize.cy += spCell->get_ExpandHeight();
            }
        }

        RECT rcViewWnd; memset(&rcViewWnd, 0, sizeof(RECT));
        if(NULL != m_hViewWnd) GetClientRect(m_hViewWnd, &rcViewWnd);
        m_offscreenSize.cy = max(m_offscreenSize.cy, (rcViewWnd.bottom - rcViewWnd.top));

        return TRUE;
    }

    BOOL ReSetCellPosition()
    {
        for(std::vector<smart_ptr<TCell>>::iterator it=m_cells.begin();
            it!=m_cells.end(); ++it)
        {
    ...
    void AddViewCell(smart_ptr<TCell> spCell)
...
protected:
    ...
    {
        YLIB::YPaint::FillRectWithColor(m_offscreenDC, RGB(255,255,255), m_offscreenSize.cx, m_offscreenSize.cy);
        for(std::vector<smart_ptr<TCell>>::iterator it=m_cells.begin();
...
public:
    ...
    std::vector<smart_ptr<TCell>> m_cells;
