...
BOOL sm_GetNtProcessInfo(const DWORD dwPID, smPROCESSINFO *ppi)
{
	...
	smPROCESSINFO *spi						= new smPROCESSINFO;
	...
	ZeroMemory(spi, sizeof(smPROCESSINFO));
...
#endif // READ_ALL_INFO
		spi->dwPEBBaseAddress = (DWORD_PTR)pbi->PebBaseAddress;
#ifdef READ_ALL_INFO
...
//				}
		...
		if(spi->dwPID == 4) {
			ExpandEnvironmentStrings(_T("%SystemRoot%\\System32\\ntoskrnl.exe"),
									 spi->szImgPath, _countof(spi->szImgPath));
		}
	}	// Read Basic Info
