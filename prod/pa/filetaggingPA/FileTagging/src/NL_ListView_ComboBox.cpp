	...
	for(int i = 0; i < nColCount; i++)//Draw all the cell one by one (column)
	{
		...
		else if(wcscmp(col.pszText, NL_LISTVIEW_COMBOBOX) == 0)
		{
			...
			{//User has inserted a combobox.
				...
				HWND hWnd = ::CreateWindowW(L"ComboBoX", L"", WS_CHILD | WS_VSCROLL | WS_VISIBLE | CBS_HASSTRINGS | nStyle,
											rc.left,
											rc.top,
											(rc.right - rc.left),
											nHeight,
				...
				if(hWnd)
				{
					...
					if(nStyle == CBS_DROPDOWNLIST)
					{
						...
						int nCount = (int)SendMessage(param.hWnd, CB_GETCOUNT, 0, 0);

						
						BOOL bDefault = FALSE;
						for(int j = 0; j < nCount; j++)
						{
							wchar_t szBuffer[COMBO_VALUE_LENGTH] = {0};
							SendMessage(param.hWnd, CB_GETLBTEXT, j, (LPARAM)szBuffer);
							if( wcscmp(param.strDefaultValue.c_str(), szBuffer) == 0)
							{
								SendMessage(param.hWnd, CB_SETCURSEL, j, 0);
								bDefault = TRUE;
								break;
							}
						}

						if(!bDefault && param.vValues.size() > 0)
						{
					...
					{
						::SetWindowTextW(param.hWnd, param.strDefaultValue.c_str());
					}
...
void CNL_ListView_ComboBox::InsertComboBox(int nItem, int nSubItem, std::vector<std::wstring>& vValues)
{
	wchar_t szKey[100] = {0};
...
BOOL CNL_ListView_ComboBox::GetItemTextEx(int nItem, int nSubItem, LPTSTR lpszText, int nLen)
{
	...
	else if(wcscmp(col.pszText, NL_LISTVIEW_COMBOBOX) == 0)//Try to get the current selected text in combobox.
	{
		...
		{//User has inserted a combobox.
			...
			if(param.hWnd)
			{
				::GetWindowTextW(param.hWnd, lpszText, nLen);
				if(wcslen(lpszText) > 0)
					return TRUE;
			}

			//can't get value from combobox, maybe the combobox was not created. for example, there are a lot of items and there is a scroll bar in the list
			if(!param.strDefaultValue.empty())
			{
				wcsncpy_s(lpszText, nLen, param.strDefaultValue.c_str(), _TRUNCATE);
				return TRUE;
			}
			else
			{
				if(param.vValues.size() > 0)//Use the first value if there ins't default value.
				{
					wcsncpy_s(lpszText, nLen, param.vValues[0].c_str(), _TRUNCATE);
					return TRUE;
				}
			}
		}
...
void CNL_ListView_ComboBox::SetComboBoxValue(int nIndex, LPCWSTR pszText)
{
	if(!pszText || nIndex >= GetItemCount())
	{
		return;
	}

	wchar_t szKey[100] = {0};
	_snwprintf_s(szKey, 100, _TRUNCATE, L"%d%d", nIndex, 1);

	std::map<std::wstring, PARAM_COMBOBOX>::iterator iter = m_mapComboBoxes.find(szKey);//Try to find the related Combobox
	if(iter != m_mapComboBoxes.end())
	{
		PARAM_COMBOBOX& param = m_mapComboBoxes[szKey];
		param.strDefaultValue = pszText;
	}
}
