...
INT CProviderInstall::GetCurrentFilePath(wchar_t *o_pathName)
{
	...
	else
	{
		...
		{
			fileName = fileName+1 ;
		iRet = (INT)wcslen(	o_pathName ) ;
			INT iLen = (INT)wcslen(fileName) ;
		iRet = iRet-iLen ;
...
{
	WSAPROTOCOL_INFOW   HPE_LayeredProtocolInfo;
	...
	for(dwIndex = 0; dwIndex < dwTotalProtocols; dwIndex++)
	{
		...
		{
			memcpy(&HPE_LayeredProtocolInfo, &lpAllProtocolInfo[dwIndex], sizeof(WSAPROTOCOL_INFOW));
			HPE_LayeredProtocolInfo.ProtocolChain.ChainLen = BASE_PROTOCOL;
			wcsncpy_s(HPE_LayeredProtocolInfo.szProtocol, WSAPROTOCOL_LEN, L"Nextlabs HPE Provider", _TRUNCATE);
			break;
		}
	}
...
#if defined(_WIN64)
	GetCurrentProcessName(pathProvider, 1023, ::GetModuleHandle(L"HPE.DLL"));
#else
	GetCurrentProcessName(pathProvider, 1023, ::GetModuleHandle(L"HPE32.DLL"));
#endif
	if(WSCInstallProvider(&HPE_LAYER_GUID, pathProvider, &HPE_LayeredProtocolInfo, 1, &nErrno) == SOCKET_ERROR)
	{
	...
	{
		DP((L"GlobalAlloc int installfilter Error \n"));
		FreeFilter(lpAllProtocolInfo);
		return FALSE;
	}
...
{
	int   errorcode;
	if(WSCDeinstallProvider(&HPE_LAYER_GUID, &errorcode) == SOCKET_ERROR)
	{
