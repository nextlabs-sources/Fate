     ...
     int    cchWideChar)
{
    if (pszAnsiBuff)
    {
        int iRet = MultiByteToWideChar(
            ...
            (int)(strlen(pszAnsiBuff) + 1), // number of bytes in string, cast to int for windows64 version to suppress warning. in windows64, int and long are 32 bits, size_t is 64 bits
...
static TCHAR* getFileAccessString(DWORD access) {
	...
	wcsncpy_s(cResult, result.length() + 1, result.c_str(), _TRUNCATE);
    ...
    while (start != NULL) {
        ...
        *end = L'\0';
        ...
        if ((wcslen(dir) + wcslen(start)) < MAX_PATH) {
            ...
            g_log.Log(CELOG_DEBUG, L"ParseDialogFileList: file name too long, ignored\n");
        }
...
void UAC_Reset();//Release INLEDPManager, so that user needs to type in the user name and password to elevate the privilege again.
