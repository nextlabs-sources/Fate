...
void GPOTransformGenerator::modifyCopiedMsi() {
	...
	wchar_t* icenetLocation;
	wchar_t* icenetHostname;
	wchar_t* icenetPortAsString;
	MSIHANDLE msiDatabase;
	MSIHANDLE propertyTableView,propertyTableView_enable_start;
	MSIHANDLE icenetLocationRecord;
	...
	icenetHostname = this->customizationParameters->getIcenetHostname();
	icenetLocation = new wchar_t[wcslen(icenetHostname) + 6 + 1];
	wcscpy_s(icenetLocation, wcslen(icenetHostname) + 6 + 1, icenetHostname);
	wcscat_s(icenetLocation, wcslen(icenetHostname) + 6 + 1, L":");
	icenetPortAsString = new wchar_t[5+1];
	wsprintf(icenetPortAsString, L"%i", this->customizationParameters->getIcenetPort());
	wcscat_s(icenetLocation, wcslen(icenetHostname) + 6 + 1, icenetPortAsString);
	icenetLocationRecord = MsiCreateRecord(1);
	result = MsiRecordSetString(icenetLocationRecord, 1, icenetLocation);
	if (result != ERROR_SUCCESS) {
		throw runtime_error("Unable to create icenet location database record.  Result: " + result);
	}

	result = MsiDatabaseOpenView(msiDatabase, 
			 L"INSERT INTO Property (Value, Property) VALUES (?, 'ICENET_SERVER_LOCATION')", 
			 &propertyTableView); 
      if (result != ERROR_SUCCESS) {
		throw runtime_error("Unable to open Property table in copied msi database.  Result: " + result);
	}
	
	result = MsiViewExecute(propertyTableView, icenetLocationRecord);
	if (result != ERROR_SUCCESS) {
		throw runtime_error("Unable to insert icenet location property record.  Result: " + result);
	}

	//wcout << "ICENET change completed." <<"\n";

       result = MsiDatabaseOpenView(msiDatabase, 
			 L"UPDATE Property set Value='0' where Property='ENABLE_SERVICE_START_ON_INSTALL'", 
			 &propertyTableView_enable_start);
       //wcout << "setupOpenView:" << result <<"\n";	
	...
	result = MsiViewExecute(propertyTableView_enable_start, 0);
	if (result != ERROR_SUCCESS) {
	    wcout << "Execute property" <<"\n";
		throw runtime_error("Unable to update property record.  Result: " + result);
	}
	...
	delete []icenetLocation;
	delete []icenetPortAsString;

	MsiCloseHandle(msiDatabase);
	MsiCloseHandle(propertyTableView);
      MsiCloseHandle(propertyTableView_enable_start);
	MsiCloseHandle(icenetLocationRecord);
}
void GPOTransformGenerator::createTransorm() {
	...
	result = MsiDatabaseGenerateTransform(copiedMsiDatabase, originalMsiDatabase, L"PolicyControllerGPOTransform.mst", 0, 0);
	if (result != ERROR_SUCCESS) {
		throw runtime_error("Unable to create msi transform.  Result: " + result);
	}

	result = MsiCreateTransformSummaryInfo(copiedMsiDatabase, originalMsiDatabase, L"PolicyControllerGPOTransform.mst", 0, MSITRANSFORM_VALIDATE_UPDATEVERSION);
	if (result != ERROR_SUCCESS) {
...
void GPOTransformGenerator::deleteMsiCopy() {
	(void)_wremove(this->getPathToMsiCopy());
}
...
wchar_t* GPOTransformGenerator::getPathToMsiCopy() {
	...
	if (this->pathToMsiCopy == NULL) {
		...
		wcscpy_s(this->pathToMsiCopy, pathToMsiLength + 5 + 1, this->pathToMsi);
		startOfNameModification = this->pathToMsiCopy + pathToMsiLength - 4;
		wcscpy_s(startOfNameModification, 5+1+4, L"-copy.msi");
	}
