...
//	add in Dec, 23th, 2009,
//	use explicit load mode.
#define REG_KEY_INSTALL_PATH   "SOFTWARE\\NextLabs\\Enterprise DLP\\Network Enforcer"
#define REG_KEY_INSTALL        L"InstallDir"
#define GZIP_HELPER_ZILB_NAME	L"zlib1.dll"
#define GZIP_HELPER_ZILB_NAME_X64	L"zlibwapi.dll"

//	are designed to only instanced once
class CZLibLoader
{
public:
	CZLibLoader();
...
private:
	...
	typedef uLong (* crc32_type) OF((uLong crc, const Bytef *buf, uInt len));
	typedef int (* inflateInit2__type) OF((z_streamp strm, int  windowBits, const char *version, int stream_size));
	typedef int (* inflate_type) OF((z_streamp strm, int flush));
	typedef int (* inflateReset_type) OF((z_streamp strm));
	typedef int (* inflateEnd_type) OF((z_streamp strm));

public:
	//	can be used by user
	crc32_type m_p_crc32;
	inflateInit2__type m_p_inflateInit2_;
	inflate_type m_p_inflate;
	inflateReset_type m_p_inflateReset;
	inflateEnd_type m_p_inflateEnd;
};

extern CZLibLoader g_ZlibLoader;
...
class CGZIP2AT
{
  ...
  void Init()
  {
		if(!g_ZlibLoader.IsInited())
		{
			//	zlib dll is not loaded well.
			//	something wrong.
			return;
		}
	if(m_gzip==0)
	...
	m_z_eof = 0;
	m_transparent = 0;
		m_crc = g_ZlibLoader.m_p_crc32(0L, Z_NULL, 0);
    
	m_zstream.next_in  =m_inbuf = (Byte*)ALLOC(Z_BUFSIZE);

		//int  err = inflateInit2(&(m_zstream), -MAX_WBITS);
		int err = g_ZlibLoader.m_p_inflateInit2_((&(m_zstream)), (-MAX_WBITS), ZLIB_VERSION, sizeof(z_stream));
 ...
 int gzread(char* buf,int len)
 {
    ...
    if (m_zstream.avail_in == 0 && !m_z_eof)
	{
          ...
          m_zstream.next_in = m_inbuf;
			...
			m_z_err = g_ZlibLoader.m_p_inflate(&(m_zstream), Z_NO_FLUSH);
	   if (m_z_err == Z_STREAM_END)
	   {
	    /* Check CRC and original size */
				m_crc = g_ZlibLoader.m_p_crc32(m_crc, start, (uInt)(m_zstream.next_out - start));
	    ...
	    }else
		{
		   ...
		   if (m_z_err == Z_OK)
		   {
		    ...
		    uLong total_out = m_zstream.total_out;
						g_ZlibLoader.m_p_inflateReset(&(m_zstream));
 ...
 int destroy()
 {
    ...
    int err = Z_OK;
	if (m_zstream.state != NULL) {
			err = g_ZlibLoader.m_p_inflateEnd(&(m_zstream));
	}
    if (m_z_err < 0) err = m_z_err;
    TRYFREE(m_inbuf);
    return err;
