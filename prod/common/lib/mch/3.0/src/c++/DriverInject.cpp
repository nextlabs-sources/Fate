// ***************************************************************
//  DriverInject.cpp          version: 1.0.0   date: 2010-01-10
//  -------------------------------------------------------------
...
BOOL SendDriverCommand(LPCWSTR driverName, DWORD command, TDrvCmdHeader *buf)
{
  WCHAR arrChW[80];
  HANDLE fh;
  ...
  if (fh != INVALID_HANDLE_VALUE)
  {
    ...
    result = DeviceIoControl(fh, command, buf, buf->Size, NULL, 0, &c1, NULL) != false;
    CloseHandle(fh);
...
SYSTEMS_API BOOL WINAPI StopInjectionDriver(LPCWSTR driverName)
{
  ...
  allow_.Allow = true;
  stopAllowed = SendDriverCommand(driverName, Ioctl_AllowUnload, &allow_.Header) != false;
  ...
  if ((!result) && (stopAllowed))
  {
    ...
    allow_.Allow = false;
    SendDriverCommand(driverName, Ioctl_AllowUnload, &allow_.Header);
  }
...
BOOL JectDll(LPCWSTR driverName, DWORD command, LPCWSTR dllFileName, DWORD session, BOOL systemProcesses, LPCWSTR includeMask, LPCWSTR excludeMask)
{
  ...
  if (dll)
  {
    ...
    if (excludeMask)
      ...
      dest[0] = L'\0';
    result = SendDriverCommand(driverName, command, &dll->Header);
    LocalFree(dll);
