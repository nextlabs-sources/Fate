...
bool CESDKConnect()
{
  ...
  if( res != CE_RESULT_SUCCESS ) {
    CELOG_LOG(CELOG_DEBUG,L"CESDKConnect: CECONN_DLL_Activate failed err=%d\n", res);
    return false;
  }
				 ...
				 NLSE_MESSAGE *outMsg)
{
  ...
  WCHAR wKeyRingName[NLSE_KEY_RING_NAME_MAX_LEN] = {0};
  
  //set key ring name
  MultiByteToWideChar(CP_UTF8, 0, 
		      inMsg->keyRingName, NLSE_KEY_RING_NAME_MAX_LEN,
		      wKeyRingName, NLSE_KEY_RING_NAME_MAX_LEN);
