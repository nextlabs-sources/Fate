		   ...
		   int outBufLen)
{
  ...
  ret_val = iconv(cd, &in, &inLen, &outBuf, &outLen);
  if(ret_val == -1) {
...
nlint32 nlstrncmp (const nlchar * s1, const nlchar * s2, size_t len)
{
...
nlint32 nlstrnicmp (const nlchar * s1, const nlchar * s2, size_t len)
{
...
size_t nlstrlen (const nlchar * str)
{
  return strlen(str);
}

size_t nlstrblen (const nlchar * str)
{
  return strlen(str);
}
...
//"ascii_buf_size" is the size of buffer "asci_buf"
bool nlstrtoascii(const nlchar *nl_str, char *ascii_buf, unsigned int ascii_buf_size)
{
...
void nlstrntolow(nlchar *dest, size_t destLength,
		 const nlchar *src, size_t srcLength)
{
