...
static void VerticalGradient(HDC hDC, const RECT& GradientFill,  
...
public:
	CNXTLBS_Window()
	{
		...
		m_nRoundWidthShadow = 50;
		m_nRoundHeightShadow = 50;

		m_clrBk = RGB(240, 241, 247);
		m_clrShadow = RGB(110, 110, 110);
...
// 	END_MSG_MAP()
	...
	virtual void DrawBk(HDC hdc)
	{
		...
		//Draw border
		HPEN hPen = CreatePen(PS_SOLID, 1, m_clrBorder);
		HPEN hOldPen = (HPEN)SelectObject(hdc, hPen);

		rc.left += m_nBorderLeftMargin;
		rc.top += m_nBorderTopMargin;
		rc.right -=  m_nBorderRightMargin;
		rc.bottom -= m_nBorderBottomMargin;
		RoundRect(hdc, rc.left, rc.top, rc.right, rc.bottom, m_nRoundWidth, m_nRoundHeight);
		DeleteObject(hPen);
		...
		if(m_bVGradientBk)
		{
			VerticalGradient(hdc, rc, m_clrTop, m_clrBottom);
		}
		else
		{
	...
	void SetVGradientBk(COLORREF clrTop, COLORREF clrBottom)
	{
		m_bVGradientBk = TRUE;
		m_clrTop = clrTop;
