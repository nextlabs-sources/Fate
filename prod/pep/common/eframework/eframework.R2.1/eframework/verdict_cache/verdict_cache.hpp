...
namespace nextlabs
{
  ...
  class verdict_cache : boost::noncopyable
  {
	...
	std::map<std::wstring,nextlabs::cache_entry>::iterator it = elem_map.find(source_lower);
	if( it != elem_map.end() )
	{
	...
	std::wstring source_lower(in_source);
	boost::algorithm::to_lower(source_lower);
	std::map<std::wstring,nextlabs::cache_entry>::const_iterator it = elem_map.find(source_lower);
	if( it != elem_map.end() )
	{
	  return true;
	}
	...
	{
	  std::map<std::wstring,nextlabs::cache_entry>::iterator it;
	  it = std::min_element(elem_map.begin(),elem_map.end(),min_touch_time_pred());
    ...
    private:
      ...
      std::map<std::wstring,nextlabs::cache_entry> elem_map; /* cache elements */
